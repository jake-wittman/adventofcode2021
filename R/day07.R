...
#' @export
f07b <- function(x) {
  fuel_cost <- function(target) sum(map2_int(x, target, ~sum(seq_len(abs(.x - .y)))))
  optimal_position <- optimize(fuel_cost, interval = c(0, 100000))
  rounded_optim <- round(optimal_position$minimum, digits = 0)
  fuel_cost(rounded_optim)
}
