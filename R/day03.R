...
#' f03b()
f03a <- function(x) {
  bits <- do.call('rbind', strsplit(x, ""))
  bits <- matrix(as.numeric(bits), ncol = ncol(bits))
  avg_bits <- colMeans(bits)
  gamma <- paste(ifelse(avg_bits > 0.5, "1", "0"), collapse = "")
  epsilon <- paste(ifelse(avg_bits < 0.5, "1", "0"), collapse = "")
...
f03b <- function(x) {
  bits <- do.call('rbind', strsplit(x, ""))
  bits_o2 <- matrix(as.numeric(bits), ncol = ncol(bits))
  bits_co2 <- bits_o2
  avg_bits_o2 <- colMeans(bits_o2)
  avg_bits_co2 <- colMeans(bits_co2)
  for (i in 1:length(avg_bits_o2)) {
    
    if (avg_bits_o2[i] < 0.5) {
      bits_o2 <- bits_o2[bits_o2[, i] == 0, ] # If 0 is most common, keep 0s
      if (is.vector(bits_o2) == TRUE) break
      avg_bits_o2 <- colMeans(bits_o2)
      
    } else if (avg_bits_o2[i] > 0.5) {
      bits_o2 <- bits_o2[bits_o2[, i] == 1, ] # if 1 is most common, keep 1
      if (is.vector(bits_o2) == TRUE) break
      avg_bits_o2 <- colMeans(bits_o2)
      
    } else if (avg_bits_o2[i] == 0.5) {
      bits_o2 <- bits_o2[bits_o2[, i] == 1, ] # if equal, keep 1
      if (is.vector(bits_o2) == TRUE) break
      avg_bits_o2 <- colMeans(bits_o2)
    }
    if (is.vector(bits_o2) == TRUE) break
  ...
  for (i in 1:length(avg_bits_co2)) {  
    
    if (avg_bits_co2[i] < 0.5) {
      bits_co2 <- bits_co2[bits_co2[, i] == 1, ] # If 0 is most common, keep 1s
      if (is.vector(bits_co2) == TRUE) break
      avg_bits_co2 <- colMeans(bits_co2)
    } else if (avg_bits_co2[i] > 0.5) {
      bits_co2 <- bits_co2[bits_co2[, i] == 0, ] # If 1 is most common, keep 0
      if (is.vector(bits_co2) == TRUE) break
      avg_bits_co2 <- colMeans(bits_co2)
    } else if (avg_bits_co2[i] == 0.5) {
      bits_co2 <- bits_co2[bits_co2[, i] == 0, ] # If equal, keep 0
      if (is.vector(bits_co2) == TRUE) break
      avg_bits_co2 <- colMeans(bits_co2)
    }
    if (is.vector(bits_co2) == TRUE) break
  ...
  o2 <- strtoi(paste(bits_o2, collapse = ""), base = 2)
  co2 <- strtoi(paste(bits_co2, collapse = ""), base = 2)
  o2 * co2
  }
