...
#' f01b()
f01a <- function(x) {
  if (is.numeric(x) == FALSE) stop('x should be a numeric vector')
  x <- tibble(x = x)
  x %>% 
    mutate(change = case_when(x > lag(x) ~ 'increase',
                              x <= lag(x) ~ 'decrease')) %>% 
    group_by(change) %>% 
    count() %>% 
...
f01b <- function(x) {
  ...
  x <- tibble(x = x)
  x %>% 
    mutate(sum_x_3 = x + lead(x) + lead(x, n = 2)) %>% 
    mutate(change = case_when(sum_x_3 > lag(sum_x_3) ~ 'increase',
                              sum_x_3 <= lag(sum_x_3) ~ 'decrease')) %>% 
    group_by(change) %>% 
    count() %>% 
    filter(change == 'increase') %>% 
    pull(n)
...
example_data_01 <- function(example = 1) {
  l <- list(
    ...
    b = c(607, 618, 618, 617, 647, 716, 769, 792)
  )
  l[[example]]
}
