...
#' f02b()
f02a <- function(x) {
  split_text <- str_split(x, pattern = " ")
  directions <- unlist(map(split_text, 1))
  magnitude <- as.numeric(unlist(map(split_text, 2)))
  # Set proper sign of magnitude
  magnitude[directions == 'up'] <- magnitude[directions == 'up'] * -1
  # Get final x and y positions
  final_x <- sum(magnitude[directions == 'forward'])
  final_y <- sum(magnitude[directions %in% c('up', 'down')])
  final_x * final_y
}
...
f02b <- function(x) {
  ...
  for (i in 1:length(directions)) {
    if (directions[i] == 'forward') {
      overall_x <- overall_x + magnitude[i]
      overall_y <- overall_y + magnitude[i] * aim
    }
    if (directions[i] %in% c('up', 'down')) {
      aim <- aim + magnitude[i]
    }
  }
